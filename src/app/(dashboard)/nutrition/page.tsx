...
export default function NutritionPage() {
  ...
  // Get user ID for nutrition logs query
  if (!userId) {
  ...
  const { data: todayLogs, isLoading } = useNutritionLogs(userId ?? "", today)
      ...
      {state === "editing" && (
        <Card>
          <CardContent className="p-4 space-y-4">
            {previewUrl && (
              <img src={previewUrl} alt="Meal" className="h-32 w-full rounded-lg object-cover" />
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Input type="number" step="0.1" value={editProtein} onChange={(e) => setEditProtein(e.target.value)} />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Carbs (g)</Label>
                <Input type="number" step="0.1" value={editCarbs} onChange={(e) => setEditCarbs(e.target.value)} />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Fat (g)</Label>
                <Input type="number" step="0.1" value={editFat} onChange={(e) => setEditFat(e.target.value)} />
              </div>
            </div>
...
function MacroCard({ icon, label, value, unit }: { icon: React.ReactNode; label: string; value: number; unit: string }) {
  return (
    <Card>
